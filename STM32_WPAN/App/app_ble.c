 ...
 */
uint8_t ad_data[25] = {
    10, AD_TYPE_COMPLETE_LOCAL_NAME, 'A', 'I', '-', 'M', 'o', 't', 'i', 'o', 'n',  /* Complete name */

    13, AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x01, 0x83, 0x00 /* No feature for BlueST SDK 31-24 */, 0x00 /* No feature for BlueST SDK 23-16 */, 0x00 /* No feature for BlueST SDK 15-08 */, 0x10 /* Feature: Motion intensity, Activity, Carry Position, MemsGesture, Pedometer */, 0x08 /* Arbitrary BLE address */, 0xDE /*  */, 0xEE /*  */, 0xAD /*  */, 0xBE /*  */, 0xEF /*  */,
};
...
/* USER CODE BEGIN APP_BLE_Init_3 */
  if(app_ism_motion_init(1) != BSP_ERROR_NONE){
    Error_Handler();
  }
  APP_DBG_MSG("ISM330DHCX init complete \r\n");
  
  /** Ref. https://github.com/STMicroelectronics/BlueSTSDK_Android
   * Formatting the advertising packet for the BlueST SDK
   * Protocol Version + Device Id + Feature Mask + Device MAC
   */
  const uint8_t * p_addr = BleGetBdAddress();
  for(uint32_t k = 0 ; k < BD_ADDR_SIZE_LOCAL; k++){
    ad_data[sizeof(ad_data) - k - 1] = p_addr[k];
  }
